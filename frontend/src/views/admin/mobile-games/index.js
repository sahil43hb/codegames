...
export default function MobileGame() {
    ...
    const [mobileGameProducts, setMobileGameProducts] = useState('');
    ...
    useEffect(() => {
        const getMobileGameProducts = async () => {
            const response = await myAxios.get('/admin/get-products-with-module-name', { params: { moduleName: 'MobileGame' } });
            if (response.status === 200) {
                setMobileGameProducts(response.data.data.results)
                setLoading(false)
            } else {
        ...
        getMobileGameProducts()
    }, []);

    const handleRemove = async (product_id, index) => {
        const response = await myAxios.post('/admin/delete-Product-with-module-name', { id: product_id }, { params: { moduleName: 'MobileGame' } });
        const filterArray = mobileGameProducts
        filterArray.splice(index, 1)
        setMobileGameProducts([...filterArray]);
                        ...
                        <Grid>
                            ...
                            <TableContainer>
                                                                ...
                                                                <IconButton
                                                                    onClick={() =>
                                                                        navigate('/admin/update-multi-denominations', {
                                                                            state: { multiProduct: row, moduleName: 'MobileGame', navigationUrl: '/admin/mobile-games' }
                                                                        })
                                                                    }
                                                                >
                                ...
                                <TablePagination
                                    ...
                                    component="div"
                                    count={mobileGameProducts.length}
