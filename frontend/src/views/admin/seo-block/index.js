...
import { Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, Grid, CircularProgress, Typography, useTheme, Tooltip, IconButton } from '@mui/material';
...
export default function SeoBlock() {
    ...
    useEffect(() => {
        const getSeoBlocks = async () => {
            const response = await myAxios.get('/admin/get-seo-block');
    ...
    const handleDelete = async (seo_block_id, index) => {
        const response = await myAxios.post('/admin/delete-seo-block', { id: seo_block_id });
        const filterArray = seoBlocks
        filterArray.splice(index, 1)
        setSeoBlocks([...filterArray])
    ...
    return (
        ...
        <MainCard
            ...
            title="SEO Blocks"
            secondary={
                <Button variant="outlined" onClick={() => navigate('/admin/add-seo-block', { state: { seoBlocks: seoBlocks } })}
                    ...
                    (
                        <Grid>
                            ...
                            <TableContainer>
                                        ...
                                        {seoBlocks
                                            ...
                                            .map((row, index) => (
                                                <TableRow key={row._id} sx={{ py: 3 }} hover role="checkbox" tabIndex={-1}>
                                                    ...
                                                    <TableCell sx={{ textAlign: 'center' }}>{row.description.split(' ').slice(0, 5).join(' ')}...</TableCell>
                                                    <TableCell sx={{ textAlign: 'center' }}>{row.sort}</TableCell>
                                                    <TableCell sx={{ textAlign: 'center' }} ><img src={row.image} style={{ width: '100px', height: '50px', borderRadius: 8 }} /></TableCell>
                                                    <TableCell sx={{ textAlign: 'center' }}>
                                                        <Tooltip title="Edit" placement="top">
                                                            <IconButton
                                                                onClick={() => navigate('/admin/edit-seo-block', { state: { seoBlock: row, seoBlocks: seoBlocks } })}
                                                            >
                                ...
                                <DeleteModal open={open} handleOpen={handleOpen} handleClose={handleClose} moduleName={'SEO Block'} handleDelete={handleDelete} index={currentIndex} id={currentId} modalLoading={modalLoading} setModalLoading={setModalLoading} />
                                <TablePagination
                                    rowsPerPageOptions={[10, 25, 100]}
                                    component="div"
                                    count={seoBlocks.length}
