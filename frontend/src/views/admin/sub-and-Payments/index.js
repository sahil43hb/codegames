...
import { useTheme, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, Grid, CircularProgress, Typography, Chip, Tooltip, IconButton } from '@mui/material';
...
export default function SubAndPaymentCard() {
    const theme = useTheme();
    const navigate = useNavigate();
    const [subPayCardProducts, setSubPayCardProducts] = useState('');
    ...
    useEffect(() => {
        ...
        const getSubPayCardProducts = async () => {
            const response = await myAxios.get('/admin/get-products-with-module-name', { params: { moduleName: 'SubAndPaymentCard' } });
            if (response.status === 200) {
                setSubPayCardProducts(response.data.data.results)
                setLoading(false)
            } else {
        ...
        getSubPayCardProducts()

    }, []);

    const handleRemove = async (product_id, index) => {
        const response = await myAxios.post('/admin/delete-Product-with-module-name', { id: product_id }, { params: { moduleName: 'SubAndPaymentCard' } });
        const filterArray = subPayCardProducts
        filterArray.splice(index, 1)
        setSubPayCardProducts([...filterArray]);
    ...
    return (
        ...
        <MainCard
            content={false}
            title="Subscribtion & Payment Cards"
        >
            {!loading ?
                (subPayCardProducts.length > 0 ?
                    (
                        <Grid>
                            {/* table */}
                            <TableContainer>
                                                                ...
                                                                <IconButton
                                                                    onClick={() =>
                                                                        navigate('/admin/update-multi-denominations', {
                                                                            state: { multiProduct: row, moduleName: 'SubAndPaymentCard', navigationUrl: '/admin/sub-and-payments' }
                                                                        })
                                                                    }
                                                                >
                                ...
                                <DeleteModal open={open} handleOpen={handleOpen} handleClose={handleClose} moduleName={'Sub & Payment Card'} handleDelete={handleRemove} index={currentIndex} id={currentId} modalLoading={modalLoading} setModalLoading={setModalLoading} />
                                {/* table pagination */}
                                <TablePagination
                                    rowsPerPageOptions={[10, 25, 100]}
                                    component="div"
                                    count={subPayCardProducts.length}
