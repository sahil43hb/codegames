...
const UpdateMultiDenomination = () => {
    const theme = useTheme();
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const location = useLocation();
    const { multiProduct, moduleName, navigationUrl } = location.state;

    const [parentData, setParentData] = useState({
        _id: multiProduct._id,
        name: multiProduct.name,
        description: multiProduct.description,
        thumnailImage: multiProduct.thumnailImage,
        coverImages: multiProduct.coverImages,
    });
    ...
    const [denominationData, setDenominationData] = useState(multiProduct.products)
    ...
    const handleFinalSubmit = async () => {
        setLoading(true);
        const response = await myAxios.post('/admin/update-denominations-product-with-module-name', { products: denominationData }, { params: { _id: multiProduct._id, moduleName: moduleName } });
        if (response && response.status === 200) {
            setLoading(false);
...
export default UpdateMultiDenomination;
